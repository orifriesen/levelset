...
def read_dat(city, race, sc_type):
    dat_path_rel = 'data/sc/' + sc_type + '/' + race + '/' + city + '.dat'
    dat_path = os.path.join(os.path.dirname(__file__), dat_path_rel)
    entry_times_rel = 'data/sc/' + sc_type + '/' + race + '/' + city + '_entry_times.csv'
    entry_times_path = os.path.join(os.path.dirname(__file__), entry_times_rel)
...
def get_vertex_coords_and_colors(vertices, city, race, sc_type, entry_times):
    key_path_rel = 'data/keys/' + sc_type + '/' + race + '/' + city + '-key'
    key_path = os.path.join(os.path.dirname(__file__), key_path_rel)
    precinct_key = {}
    ...
    vertex_coords = {}
    if sc_type != 'ls':
        centroid_path = '../data/extractcentroids/' + city + '.csv'
        ...
        img_path_rel = 'data/tif/' + race + '/' + city + '.tif'
        img_path = os.path.join(os.path.dirname(__file__), img_path_rel)
...
def visualize_simplicial_complex(city, race, sc_type, animate = False):
    vertices, edges, faces, entry_times = read_dat(city, race, sc_type)
    precinct_key, vertex_coords, vertex_colors = get_vertex_coords_and_colors(vertices, city, race, sc_type, entry_times)
    ...
    if animate:
        ...
        for T in range(max(entry_times)+1):
            ...
            outdir_path = 'vis/sc/' + sc_type + '/' + race + '/' + city + '/'
            outdir = os.path.join(os.path.dirname(__file__), outdir_path)
        ...
        outdir_path = 'vis/sc/' + sc_type + '/' + race + '/'
        outdir = os.path.join(os.path.dirname(__file__), outdir_path)
        if not os.path.isdir(outdir):
            os.makedirs(outdir)
        outfile = outdir + city + '.png'
        plt.savefig(outfile, bbox_inches='tight')
        plt.close(fig)

def main():
    for city in ['okc', 'seattle', 'stpaul', 'pittsburgh']:
        for race in ['white','black', 'asian', 'hispanic']:
            for sc_type in ['ls']:
                visualize_simplicial_complex(city, race, sc_type, True)
